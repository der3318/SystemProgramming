...
int csiebox_client_run(csiebox_client* client) {
	// if login in fail, resend a request after 3 sec(s)
	while(login(client) == -1)	sleep(3);
...
static int login(csiebox_client* client) {
	...
	if (header.res.magic == CSIEBOX_PROTOCOL_MAGIC_RES && header.res.op == CSIEBOX_PROTOCOL_OP_LOGIN) {
...
static void sync_all(csiebox_client* client, char* longest_path, int level) {
	...
	while ((file = readdir(dir)) != NULL) {
		...
		// if sync file fail, resend the request after 3 sec(s)
		while(sync_file(client, file->d_name) == -1)	sleep(3);
...
static int sync_file(csiebox_client* client, char* path) {
	...
	if(status == CSIEBOX_PROTOCOL_STATUS_BUSY)	return -1;
	if (status == CSIEBOX_PROTOCOL_STATUS_MORE) {
		return sync_file_data(client, path);
	}
}
...
static csiebox_protocol_status sync_file_meta(csiebox_client* client, char* path) {
	...
	recv_message(client->conn_fd, &header, sizeof(header));
	// server is busy
	if(header.res.status == CSIEBOX_PROTOCOL_STATUS_BUSY)	fprintf(stderr, "Server busy\n");
	if(header.res.status == CSIEBOX_PROTOCOL_STATUS_FAIL)	fprintf(stderr, "sync meta fail: %s\n", path);
	return header.res.status;
}

static int sync_file_data(
...
static void handle_inotify(csiebox_client* client) {
	...
	while (i < len) {
		...
		if (event->mask & IN_CREATE) {
			...
			// if server is busy, resend after 3 sec(s)
			while(sync_file(client, path) == -1)	sleep(3);
			...
			// if server is busy, resend after 3 sec(s)
			while(sync_file_meta(client, path) == CSIEBOX_PROTOCOL_STATUS_BUSY)	sleep(3);
		} else if (event->mask & IN_DELETE) {
			fprintf(stderr, "type: delete\n");
			fprintf(stderr, "rm file: %s\n", path);
			// if server is busy, resend after 3 sec(s)
			while(rm_file(client, path, event->mask & IN_ISDIR) == -1)	sleep(3);
		} else {
